...
test("開閉/フォーカス復帰/フォーカストラップ/外側クリック", async ({ page }) => {
    await page.route(DETAIL, (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(detailOk) }));
...
test("404: トースト表示後に自動クローズ", async ({ page }) => {
    await page.route(DETAIL, (route) => route.fulfill({ status: 404, contentType: "application/json", body: JSON.stringify(detail404) }));
    await openTasks(page);
    const trigger = page.locator('[data-testid="task-title-1001"]');
    await expect(trigger).toBeVisible();
    await trigger.click();
...
test("5xx: トースト表示→開いたまま→再試行で復帰", async ({ page }) => {
    await page.route(DETAIL, (route) => route.fulfill({ status: 500, contentType: "application/json", body: JSON.stringify(detail5xx) }));
    await openTasks(page);
    const trigger = page.locator('[data-testid="task-title-1001"]');
    await expect(trigger).toBeVisible();
    await trigger.click();
