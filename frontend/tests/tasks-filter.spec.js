...
// ルート（親）タスク行だけを拾う
const parents = (page) => page.locator('[data-testid^="task-item-"][role="treeitem"][aria-level="1"]');
...
async function waitForTasksUI(page) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
        ...
        if (seen) {
            ...
            await page
                .waitForResponse((r) => r.request().method() === "GET" && /\/api\/tasks(\b|\/|\?)/.test(r.url()) && r.status() === 200, { timeout: 10_000 })
                .catch(() => { });
...
async function waitForTasksFetch(page, { order_by, dir, site }) {
    const needs = ["/api/tasks"];
    if (order_by)
        needs.push(`order_by=${order_by}`);
    if (dir)
        needs.push(`dir=${dir}`);
    if (site)
        needs.push(`site=${encodeURIComponent(site)}`);
    await page
