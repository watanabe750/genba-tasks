...
async function writeTmp(testInfo: any, filename: string, data: Buffer) {
...
test.describe("画像アップロード・表示フロー", () => {
  ...
  test("リスト→サムネクリック→ドロワー画像タブが開く（フォーカス戻り含む）", async ({
    ...
    await expect(
      page.locator('#drawer-image-section img[data-testid="drawer-image"]')
  ...
  test("親：添付→表示→置換→削除→再読込でも永続状態一致", async ({
    page,
  }, testInfo) => {
    const row = await openImagePanel(page);

    // 添付
    const img1 = await writeTmp(
      testInfo,
      "img1.jpg",
      Buffer.from(JPG_1x1_BASE64, "base64")
    );
    await row.getByRole("button", { name: /画像を追加|画像を変更/ }).click();
    ...
    const thumbAfterDelete = row
      ...
      .first();
    await expect(thumbAfterDelete).toHaveAttribute(
      "aria-label",
      "画像は未設定"
    );
    ...
    await expect(
      row2.getByRole("button", { name: "画像は未設定" }).first()
    ).toBeVisible();
    await expect(row2.getByRole("button", { name: "画像を表示" })).toHaveCount(
      0
    );
  });
  ...
  test("子：画像UIなし（またはdisabled）", async ({ page }) => {
    ...
    await expect(
      childRow.getByRole("button", { name: /画像を表示|画像は未設定/ })
    ).toHaveCount(0);
  });
