...
const API_ORIGIN =
  process.env.PW_API_ORIGIN || process.env.VITE_API_ORIGIN || "http://localhost:3000";
export const apiUrl = (path: string) => (path.startsWith("http") ? path : new URL(path, API_ORIGIN).toString());
...
// 401 を見たらだけ強制再ログイン
export async function waitForTasksOk(page: Page, timeout = 15_000) {
  ...
  while (Date.now() < until && !page.isClosed()) {
    ...
    try {
      resp = await page.waitForResponse(
        r => r.url().includes("/api/tasks") && r.request().method() === "GET",
        { timeout: remain }
      );
    } catch {
      break;
...
export async function refreshTasks(page: Page) {
  if (page.isClosed()) return;
  await page.evaluate(() => window.dispatchEvent(new Event("auth:refresh"))).catch(() => {});
  await waitForTasksOk(page);
}
...
async function readDtaHeaders(page: Page): Promise<Record<string, string>> {
  if (page.isClosed()) return {};
...
async function writeRotatedTokens(page: Page, headers: Record<string, string | undefined>) {
  if (page.isClosed()) return;
  await page
    .evaluate((h) => {
      if (h["access-token"]) localStorage.setItem("access-token", h["access-token"]!);
      if (h["client"]) localStorage.setItem("client", h["client"]!);
      if (h["uid"]) localStorage.setItem("uid", h["uid"]!);
      if (h["expiry"]) localStorage.setItem("expiry", h["expiry"]!);
      if (h["token-type"]) localStorage.setItem("token-type", h["token-type"]!);
    }, headers as any)
    .catch(() => {});
}

/** 必要な時だけサインイン（force か、トークン欠落時） */
export async function ensureAuthTokens(page: Page, opts?: { force?: boolean }) {
  if (page.isClosed()) return;
...
export async function createTaskViaApi(page: Page, task: any, opts?: { waitForDom?: boolean }) {
  ...
  if (waitForDom && !page.isClosed() && /\/tasks(\?.*)?$/.test(page.url())) {
    ...
    await waitForTasksOk(page);
    const createdId: number | undefined = json?.task?.id ?? json?.id;
    if (createdId) {
